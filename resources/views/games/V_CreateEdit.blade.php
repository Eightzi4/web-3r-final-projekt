...
<x-layouts.v-main-layout :title="$game->exists ? 'Edit Game: ' . $game->name : 'Add New Game'" :breadcrumbs="$breadcrumbs">
    ...
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 shadow-2xl rounded-xl p-6 sm:p-8 md:p-10">
        ...
        <form method="POST"
            ...
            }" x-init="if ({{ session()->has('errors') && old('prices') ? 'true' : 'false' }}) {
                let oldPrices = {{ json_encode(old('prices', [])) }};
                if (Array.isArray(oldPrices)) {
                    priceEntries = oldPrices.map(p => ({
            ...
            } else if (!{{ $game->exists ? 'true' : 'false' }} || {{ $game->exists && $game->prices->count() === 0 ? 'true' : 'false' }}) {
                if (priceEntries.length === 0) {
                    priceEntries.push({ platform_id: '', store_id: '', price: '', discount: '0' });
                }
            }

            if ({{ session()->has('errors') && old('tags') ? 'true' : 'false' }}) {
                let oldTagsString = {{ json_encode(old('tags', '')) }};
                if (oldTagsString) {
                    selectedTags = oldTagsString.split(',');
                }
            }">
            @csrf
            @if ($game->exists)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                    ...
                    <div>
                        <label for="developer_id"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Developer
                            *</label>
                        <select name="developer_id" id="developer_id" required
                            class="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('developer_id') border-red-500 @enderror">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                    <div>
                        <label for="trailer_link"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Trailer Link
                            (YouTube URL)</label>
                        <input type="url" name="trailer_link" id="trailer_link"
                            value="{{ old('trailer_link', $game->trailer_link ?? '') }}"
                    ...
                    <div>
                        <label for="visible"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Visibility
                            *</label>
                        <select name="visible" id="visible"
                            class="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('visible') border-red-500 @enderror"
                            required>
                            <option value="1"
                                {{ old('visible', $game->exists ? ($game->visible ? '1' : '0') : '1') == '1' ? 'selected' : '' }}>
                                Visible</option>
                            <option value="0"
                                {{ old('visible', $game->exists ? ($game->visible ? '1' : '0') : '1') == '0' ? 'selected' : '' }}>
                                Hidden</option>
                        </select>
                        @error('visible')
                            <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            ...
            <section class="space-y-2">
                <h2
                    ...
                    Tags</h2>
                <div>
                    <label for="tags-input-field"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Add or Select
                        Tags</label>
                    ...
                    <div class="flex items-center border border-gray-300 dark:border-gray-600 rounded-lg p-2">
                        <div class="flex flex-wrap gap-2 p-1 flex-grow min-h-[40px]">
                            <template x-for="tag in selectedTags" :key="tag">
                                <span
                                    class="bg-indigo-100 dark:bg-indigo-700 text-indigo-700 dark:text-indigo-200 px-3 py-1.5 rounded-full text-sm flex items-center shadow-sm">
                                    <span x-text="tag"></span>
                                    <button type="button" @click="removeTag(tag)"
                                        class="ml-2 text-indigo-500 dark:text-indigo-300 hover:text-indigo-700 dark:hover:text-indigo-100 text-lg leading-none">×</button>
                                </span>
                            </template>
                        </div>
                        <div x-data="{ openDropdown: false }" class="relative ml-2">
                            <input type="text" id="tags-input-field" x-model="newTag"
                                ...
                                @click.outside="openDropdown = false" placeholder="New tag..."
                                class="p-2.5 border-0 focus:ring-0 dark:bg-gray-700 dark:text-gray-200 rounded-md min-w-[160px]">
                            <div x-show="openDropdown && availableTags().length > 0" x-transition
                                ...
                                <template x-for="availTag in availableTags()" :key="availTag">
                                    <div @click="addTag(availTag); openDropdown = false;"
                                        class="px-3 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer text-sm"
                                        x-text="availTag"></div>
                                </template>
                            </div>
                        </div>
                        <button type="button" @click="addTag(newTag)"
                            class="ml-2 px-4 py-2.5 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 shadow-md">Add</button>
                    </div>
                    @error('tags')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Type a tag and press Enter or click Add.
                        Existing tags can be selected from the dropdown.</p>
                </div>
            </section>
            ...
            <section class="space-y-2">
                ...
                <div>
                    <label for="images-upload"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Upload New
                        Images</label>
                    <input type="file" name="images[]" id="images-upload" multiple accept="image/*"
                    ...
                    @if ($game->exists && $game->images->isNotEmpty())
                        <div class="mt-6">
                                ...
                                @foreach ($game->images as $image)
                                    <div class="relative group aspect-w-1 aspect-h-1">
                                        <img src="{{ asset('storage/images/' . $image->image) }}"
                                            alt="Game image {{ $loop->iteration }}"
                                            class="w-full h-full object-cover rounded-lg shadow-md">
                                        <div
                                            class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-60 transition-opacity flex items-center justify-center rounded-lg">
                                            <label :for="'delete_image_{{ $image->id }}'"
                                                class="hidden group-hover:flex items-center p-2 bg-red-600 text-white rounded-full cursor-pointer hover:bg-red-700 transition-colors shadow-lg">
                                                <input type="checkbox" name="delete_images[]"
                                                    value="{{ $image->id }}"
                                                    id="delete_image_{{ $image->id }}"
                                                    class="opacity-0 absolute h-0 w-0 peer">
                                                <svg class="w-5 h-5 peer-checked:hidden" fill="none"
                                                ...
                                                <svg class="w-5 h-5 hidden peer-checked:block" fill="currentColor"
                                                    viewBox="0 0 20 20">
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Select images to delete them upon
                                saving changes.</p>
                        </div>
                    @endif
                </div>
            ...
            <section class="space-y-6 border-t border-gray-300 dark:border-gray-700 pt-8">
                ...
                <div class="space-y-6">
                    <template x-for="(price, index) in priceEntries" :key="index">
                        <div
                            class="border border-gray-300 dark:border-gray-600 p-5 rounded-xl shadow-lg bg-gray-50 dark:bg-gray-700/50 space-y-4">
                            <div class="flex justify-between items-center mb-2">
                                <h4 class="font-semibold text-gray-700 dark:text-gray-300 text-lg">Price Entry #<span
                                        x-text="index + 1"></span></h4>
                                <button type="button" @click="priceEntries.splice(index, 1)"
                                    class="text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-medium px-3 py-1 rounded-md hover:bg-red-50 dark:hover:bg-red-900/50"
                                    x-show="priceEntries.length > 1">
                                    Remove
                                </button>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                <div>
                                    <label :for="'platform_' + index"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Platform
                                        *</label>
                                ...
                                <div>
                                    <label :for="'store_' + index"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Store
                                        *</label>
                                ...
                                <div>
                                    <label :for="'price_val_' + index"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Price
                                        ($) *</label>
                                    <input type="number" step="0.01" min="0" x-model="price.price"
                                        :name="`prices[${index}][price]`" :id="'price_val_' + index"
                                        class="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                        required>
                                </div>
                                ...
                                <div>
                                    <label :for="'discount_' + index"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Discount
                                        (%)</label>
                                    <input type="number" step="1" min="0" max="100"
                                        x-model="price.discount" :name="`prices[${index}][discount]`"
                                        :id="'discount_' + index" value="0"
                                        class="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                            </div>
                        </div>
                ...
                <button type="button"
                    @click="priceEntries.push({ platform_id: '', store_id: '', price: '', discount: '0' })"
